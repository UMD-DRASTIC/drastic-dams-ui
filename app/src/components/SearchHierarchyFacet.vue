<template>
  <div v-show="facet.data.length" style="text-align: left" class="sui-multi-checkbox-facet sui-facet">
    <div class="sui-multi-checkbox-facet__label">
      {{ facet.field }}
    </div>
    <div class="sui-multi-checkbox-facet__options-list">
      <label
        v-for="facetItem in sorted"
...
export default {
  ...
  computed: {
    sorted: function () {
      return this.facet.data.slice().sort((a, b) => a.value.localeCompare(b.value));
    },
  },
  methods: {
    isChecked(value) {
